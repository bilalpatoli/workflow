...
import type { QuizQuestion, TrainingPlan } from './planner';

export async function quizGenerator(sop: string, script: TrainingPlan): Promise<QuizQuestion[]> {
...
- IDs: q1, q2, q3...`,
  ...
  if (!content) throw new Error('QuizGenerator: empty response from Z.AI');
  return (JSON.parse(content) as { quiz: QuizQuestion[] }).quiz;
}
