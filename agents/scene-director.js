...
async function sceneDirector(scenes) {
...
- Cinematic, warm lighting. Shallow depth of field.`,
            ...
            {
                role: 'user',
                content: `Write visual prompts for these scenes:\n${JSON.stringify(scenes.map(s => ({ id: s.id, title: s.title, voiceover: s.voiceover })), null, 2)}`,
            },
        ],
    ...
    if (!content)
        throw new Error('SceneDirector: empty response from Z.AI');
