...
export async function scriptWriter(plan: TrainingPlan): Promise<TrainingPlan> {
...
- Do not add new facts not present in the original.`,
      },
      {
        ...
        content: `Rewrite these scene voiceovers:\n${JSON.stringify(
          plan.scenes.map(s => ({ id: s.id, title: s.title, voiceover: s.voiceover })),
          null,
          2
        )}`,
      },
  ...
  if (!content) throw new Error('ScriptWriter: empty response from Z.AI');

  const result = JSON.parse(content) as { scenes: { id: string; voiceover: string }[] };
