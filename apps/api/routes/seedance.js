...
async function downloadClip(url) {
    ...
    const path = (0, path_1.join)((0, os_1.tmpdir)(), `clip_${Date.now()}_${Math.random().toString(36).slice(2)}.mp4`);
    await (0, promises_1.writeFile)(path, Buffer.from(await res.arrayBuffer()));
    return path;
}
async function generateWithByteplus(prompt, duration) {
    ...
    if (!res.ok)
        throw new Error(`BytePlus ${res.status}: ${await res.text()}`);
    const data = (await res.json());
    const taskId = data.id ?? data.task_id;
    if (!taskId)
        throw new Error('BytePlus: no task ID in response');
    for (let i = 0; i < MAX_POLLS; i++) {
        ...
        if (status.status === 'failed')
            throw new Error(`BytePlus task ${taskId} failed`);
    }
    throw new Error('BytePlus: polling timeout');
}
async function generateWithImaRouter(prompt, duration) {
    ...
    if (!res.ok)
        throw new Error(`ImaRouter ${res.status}: ${await res.text()}`);
    const data = (await res.json());
    const taskId = data.id ?? data.task_id;
    if (!taskId)
        throw new Error('ImaRouter: no task ID in response');
    for (let i = 0; i < MAX_POLLS; i++) {
        await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
        ...
        if (status.status === 'failed')
            throw new Error(`ImaRouter task ${taskId} failed`);
    }
    throw new Error('ImaRouter: polling timeout');
}
async function generateVideo(prompt, duration = 5, narration) {
    if (narration)
        prompt = `${prompt} Narrator says: "${narration}"`;
    try {
        return await generateWithByteplus(prompt, duration);
    }
    catch (err) {
        console.warn('[seedance] BytePlus failed, falling back to ImaRouter:', err.message);
